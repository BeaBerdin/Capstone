...
<body class="min-h-screen bg-white dark:bg-neutral-950">
    ...
    <flux:sidebar sticky collapsible="mobile" class="relative z-10 w-72.5 border-e border-zinc-200 bg-zinc-50 dark:border-neutral-800 dark:bg-neutral-950/90 dark:backdrop-blur">

        <flux:sidebar.header>
            <a href="{{ route('dashboard') }}" class="flex items-center gap-4 px-5 py-6 transition hover:opacity-95">
                <div class="flex h-14 w-14 items-center justify-center rounded-2xl bg-linear-to-br from-purple-600/20 to-indigo-600/20 ring-1 ring-purple-500/20">
                    <img src="{{ asset('images/pathwise-icon.png') }}" alt="PATHWISE" class="h-11 w-11 shrink-0 drop-shadow-[0_0_18px_rgba(168,85,247,0.55)]">
                </div>
                <div class="min-w-0">
                    <div class="text-xl font-black tracking-[0.18em] text-white">PATHWISE</div>
                    <div class="mt-0.5 text-[10px] font-semibold uppercase tracking-[0.35em] text-purple-400">AN AI LEARNING PLATFORM</div>
                </div>
            </a>
        ...
        <flux:sidebar.nav>
            <flux:sidebar.group :heading="__('Main')" class="mt-2 grid gap-1 px-2">
                ...
                @if(auth()->user()->hasRole('super_admin'))
                    ...
                    <flux:sidebar.item icon="credit-card" :href="route('super_admin.transactions.index')" :current="request()->routeIs('super_admin.transactions.*')" wire:navigate>
                        {{ __('Transactions') }}
                    </flux:sidebar.item>
                ...
                @elseif(auth()->user()->hasRole('admin'))
                    ...
                    <flux:sidebar.item icon="chart-bar" :href="route('student-progress.index')" :current="request()->routeIs('student-progress.*')" wire:navigate>
                        {{ __('Student Progress') }}
                    </flux:sidebar.item>
